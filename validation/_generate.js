...
const macScreens = [
  ...
  {
    id: "10-create-event",
    name: "Create Event (Coming Soon placeholder)",
    source: ["apps/ios-macos/Sources/LikemindedMac/MacScreens.swift (createEvent)"],
    mockup: null,
    entry: ["(Currently no live entry point — community detail events are static)"],
    backend: [],
    controls: [],
    notes: "Entire screen is a 'Community-created events are coming soon.' placeholder. Zero interactive controls. Intentionally deferred per PROGRESS.md Phase 7.B.",
  },
  {
    id: "11-meet-recap",
    name: "Meet Recap",
...
function emitMD(s, platform) {
  const lines = [];
  lines.push(`# ${s.name}`);
  lines.push("");
  lines.push(`> Platform: **${platform}** · Mockup: ${s.mockup ? `\`${s.mockup}\`` : "_missing — to be authored from DESIGN.md_"} · Status: **pending**`);
  lines.push("");
  lines.push("## Source");
  s.source.forEach((f) => lines.push(`- \`${f}\``));
  lines.push("");
  lines.push("## Entry points");
  s.entry.forEach((e) => lines.push(`- ${e}`));
  lines.push("");
  lines.push("## Backend dependencies");
  if (s.backend.length === 0) lines.push("- _(none — display only)_");
  else s.backend.forEach((b) => lines.push(`- \`${b}\``));
  lines.push("");
  lines.push("## Visual parity vs mockup");
  lines.push("- Result: `pending`");
  lines.push("- Layout: _TBD_");
  lines.push("- Color: _TBD_");
  lines.push("- Typography: _TBD_");
  lines.push("");
  lines.push("## Control ledger");
  lines.push("");
  lines.push("| Control | Type | Expected | Result | Evidence |");
  lines.push("|---|---|---|---|---|");
  s.controls.forEach((c) => {
    const result = c.blocker ? "🚫 blocked" : "⏳ pending";
    const stub = /STUB/.test(c.expected) ? " ⚠️ **STUB**" : "";
    lines.push(`| ${c.label}${stub} | ${c.type} | ${c.expected} | ${result}${c.blocker ? ` — ${c.blocker}` : ""} | _TBD_ |`);
  });
  if (s.controls.length === 0) lines.push("| _(no interactive controls)_ | — | — | — | — |");
  lines.push("");
  if (s.notes) {
    lines.push("## Notes");
    lines.push(s.notes);
  ...
  lines.push("---");
  lines.push("_Generated by `validation/_generate.js`. Update `result`/`evidence` after runtime testing._");
  return lines.join("\n");
}
...
function writePlatform(screens, platform, dir) {
  screens.forEach((s) => {
    fs.writeFileSync(path.join(dir, `${s.id}.md`), emitMD(s, platform));
    fs.writeFileSync(path.join(dir, `${s.id}.json`), JSON.stringify(emitJSON(s, platform), null, 2) + "\n");
  });
}
...
// ---------- README with status table ----------

function countResults(screens) {
  // approximations from scaffold (blocked counted; rest pending)
  let pass = 0, fail = 0, blocked = 0, pending = 0, stubs = 0;
  screens.forEach((s) => {
    s.controls.forEach((c) => {
      if (c.blocker) blocked++;
      else pending++;
      if (/STUB/.test(c.expected)) stubs++;
    });
  });
  return { pass, fail, blocked, pending, stubs };
}

const iosCounts = countResults(iosScreens);
const macCounts = countResults(macScreens);
...
|---|---|---|---|---|---|---|---|
| iOS  | ${iosScreens.length} | ${iosScreens.reduce((n, s) => n + s.controls.length, 0)} | ${iosCounts.pass} | ${iosCounts.fail} | ${iosCounts.blocked} | ${iosCounts.pending} | ${iosCounts.stubs} |
| macOS | ${macScreens.length} | ${macScreens.reduce((n, s) => n + s.controls.length, 0)} | ${macCounts.pass} | ${macCounts.fail} | ${macCounts.blocked} | ${macCounts.pending} | ${macCounts.stubs} |

_Last run: not yet started._

## iOS screens

| # | Screen | Mockup | Controls | Status |
|---|---|---|---|---|
${iosScreens.map((s, i) => `| ${i + 1} | [${s.name}](ios/${s.id}.md) | ${s.mockup ? `\`${s.mockup}\`` : "❌ missing"} | ${s.controls.length} | pending |`).join("\n")}

## macOS screens

| # | Screen | Mockup | Controls | Status |
|---|---|---|---|---|
${macScreens.map((s, i) => `| ${i + 1} | [${s.name}](macos/${s.id}.md) | ${s.mockup ? `\`${s.mockup}\`` : "❌ missing"} | ${s.controls.length} | pending |`).join("\n")}
...
console.log(`Scaffolded ${iosScreens.length} iOS + ${macScreens.length} macOS screens.`);
console.log(`iOS: ${IOS}`);
