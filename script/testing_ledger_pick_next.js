...
function flowPriority(flow, data, plat, includeBlocked = false) {
...
function interleaveByScreen(items) {
  ...
  for (const arr of byScreen.values()) {
    arr.sort((a, b) => a.flow_id.localeCompare(b.flow_id));
  }
  const screens = [...byScreen.keys()].sort((a, b) => a.localeCompare(b));
  const out = [];
...
function pickNextFlow(platform, { includeBlocked = false, screen = null, flowId = null } = {}) {
  ...
  for (const file of listScreenFiles()) {
    ...
    for (const flow of data.flows || []) {
      ...
      const pri = flowPriority(flow, data, platform, includeBlocked);
      if (!pri) continue;
      candidates.push({
        screen: logicalId,
        flow_id: flow.id,
...
module.exports = { pickNextFlow, flowPriority, sortCandidates, interleaveByScreen };
